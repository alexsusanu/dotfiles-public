...
get_screenshot_path() {
    ...
    local latest_screenshot=$(ls -t "$win_screenshots"/*.{png,jpg,jpeg} 2>/dev/null | head -n 1)
...
get_recording_path() {
    ...
    local latest_recording=$(ls -t "$win_recordings"/*.{mp4,avi,mkv} 2>/dev/null | head -n 1)
